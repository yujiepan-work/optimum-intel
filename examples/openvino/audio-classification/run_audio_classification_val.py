...
ov_xml = Path('/nvme2/yujiepan/workspace/jpqd-test/LOGS/optimum-ac/audio/0110qat_QuantInput_lr3e-05_epoch12/openvino_model.xml')
compiled_model = ov.compile_model(ov_xml.as_posix())
infer_request = compiled_model.create_infer_request()
print_openvino_inference_message = [True]
def forward(
    self,
    ...
    if print_openvino_inference_message[0]:
        print('forwarding with openvino....')
        print_openvino_inference_message[0] = False
    data = input_values.cpu().numpy()
...
def main():
    ...
    trainer.model.__class__.forward = forward
    # trainer.model.load_state_dict(torch.load(ov_xml.parent / 'pytorch_model.bin', map_location='cpu'))
