...
def boxplot_by_token(benchdict, args):
    ...
    for i, (input_seqlen, raw_latency) in enumerate(benchdict.items()):
        ...
        title_str = "Input Length: {} | Generated Length: {} | beam: {} | freq: {} | host: {} | label: {}".format(
            input_seqlen, gen_seqlen, args.n_beam, freq, args.host, args.bench_label
        )
        ...
        ax.plot(
            range(1, gen_seqlen + 1),
            mean_vec,
            marker=".",
            linestyle="--",
            ms=12,
            label=f"{input_seqlen}",
            color=colors[i],
        )
    ...
    for input_seqlen, raw_latency in benchdict.items():
        ...
        ax.set_xlabel("N-th generated token", fontsize=axis_fontsize)
        ax.set_ylabel("Mean Latency (sec.)", fontsize=axis_fontsize)
        title_str = "N-th token latency over Input Length: {} \nGenerated Length: {} | beam: {} | freq: {} | host: {} | label: {}\n model: {}".format(
            str(args.sweep_length), gen_seqlen, args.n_beam, freq, args.host, args.bench_label, args.model
        )
        ax.set_title(title_str, fontsize=10)
        ax.grid(linestyle="-.", linewidth=0.5)
    ax.legend()
...
def barplot_first_token_latency(benchdict, args):
    ...
    ax = first_token_latency_df.plot.barh(y=["mean"])
    for container in ax.containers:
        ax.bar_label(container, fmt="%.3fs", padding=5)
        ax.set_xlabel("latency (sec.)")
        ax.set_ylabel("Input Length")
    ax.set_title(f"First token latency, by Prompt Length. #beam: {args.n_beam}\n model: {args.model}", fontsize=10)
...
def barplot_beyond_token1_latency(benchdict, args):
    ...
    ax = beyond_token1_latency_df.plot.barh(y=["max", "mean", "min"])
    for container in ax.containers:
        ax.bar_label(container, fmt="%.3fs", padding=5)
        ax.set_xlabel("latency (sec.)")
        ax.set_ylabel("Input Length")
    ax.set_title(
        f"Latency of 2nd token and beyond, by Prompt Length.\n Generated Length: {args.max_new_tokens}, #beam: {args.n_beam}, model: {args.model}",
        fontsize=10,
    )
...
def build_argparser():
    ...
    args.add_argument(
        "-m",
        "--model",
        help="Required. HuggingFace Model Id or local path, model must contain OV IR",
        required=True,
        type=str,
    )
    ...
    args.add_argument(
        "--max_new_tokens",
        help="Optional. Maximum number of New (generated) tokens in generation",
        default=50,
        required=False,
        type=int,
    )
    parser.add_argument(
        ...
        "--sweep_length",
        nargs="+",
        help="<Required> sweep space of input prompt length",
        required=True,
        type=int,
    )
    args.add_argument(
        "--n_sample", help="Optional. #sample of latency collection, default to 2", default=2, required=False, type=int
    )
    args.add_argument(
        "--n_warmup",
        help="Optional. warm up #loop for generation pipeline, default to 1",
        default=1,
        required=False,
        type=int,
    )
    args.add_argument("--n_beam", help="Optional. beam", default=1, required=False, type=int)
    args.add_argument(
        "-d",
        "--device",
        help="Optional. Target device to perform inference on. " "Default value is CPU",
        default="CPU",
        type=str,
    )
    ...
    args.add_argument(
        "--bench_outdir",
        help="benchmark dump output dir, local dir with the name of label will be created if not provided",
        required=False,
        type=str,
    )
...
def main():
    ...
    for ctx_len in SWEEP_PROMPT_LENGTH:
        ...
        benchmark_pipe = partial(
            pipe, prompt, max_new_tokens=GEN_MAXLEN, num_return_sequences=1, num_beams=args.n_beam
        )
